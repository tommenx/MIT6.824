...
type Raft struct {
	...
	// partB
	//index of highest lof entry have commited,depends on appendArgs's leader's Commit
	commitIndex int
	lastApplied int
	log         []LogEntry //first index is 1
	nextIndex   []int      // the server(i)'s start of logs
	matchIndex  []int      // the server(i)'s index have mached with the leader
...
type RequestVoteArgs struct {
	...
	CandidateId int
	//part B
	LastLogIndex int //index of candidate's last log enrty
...
type AppendEntriesArgs struct {
	...
	LeaderID int
	//partB
	PrevLogIndex int        //index of log entries
	PrevLogTerm  int        //term of prevLogIndex
	Entries      []LogEntry //log entries to store leader entries[prevLogIndex+1: end]
	LeaderCommit int
}
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Term < rf.currentTerm {
		...
		reply.Success = true
		rf.currentTerm = args.Term
		rf.updateStateTo(FOLLOWER) //if cur_state is candidate
	} else {
		reply.Success = true
	}
	/******log******/
	...
	if args.PrevLogIndex > rf.getLastLogIndex() {
		reply.Success = false
		reply.NextTrival = rf.getLastLogIndex() + 1
		return
	}
	//same index different term
	//find the first index of the different term
	if args.PrevLogTerm != rf.log[args.PrevLogIndex].Term {
		...
		reply.NextTrival = i + 1
		return
	}
	/******append******/
	//check if is inconsist with the leader's entries
	conflictIdx := -1
	//diff
	if rf.getLastLogIndex() < args.PrevLogIndex+len(args.Entries) {
		conflictIdx = args.PrevLogIndex + 1
	} else {
		//assume the entry with the same index and same term are same
		for idx := 0; idx < len(args.Entries); idx++ {
			if rf.log[args.PrevLogIndex+idx+1].Term != args.Entries[idx].Term {
				conflictIdx = args.PrevLogIndex + idx + 1
				break
			}
		}
	}
	//if is not consist with leader,overwritten with the new  entry
	if conflictIdx != -1 {
...
func (rf *Raft) broadcastAppendEnrties() {
	...
	sendAppendEntriesTo := func(server int) bool {
		...
		if rf.isState(LEADER) && rf.sendAppendEntries(server, &args, &reply) {
			rf.mu.Lock()
			defer rf.mu.Unlock()
			//is reply success then send update the matchIndex nextIndex
			if reply.Success == true {
				...
				if reply.Term > rf.currentTerm {
					...
					// log not match
					rf.nextIndex[server] = reply.NextTrival
					return true
				}
			}
		}
